...
/* ──────────────────────────────────────────────
   7. DOWNLOAD MODAL
   ────────────────────────────────────────────── */
const WINDOWS_DOWNLOAD_URL = 'https://drive.google.com/uc?export=download&id=1kDO0NNS3sa1sz1zVZnp7gaRio2FujOQM';
const WINDOWS_DOWNLOAD_FALLBACK_URL = 'https://drive.google.com/file/d/1kDO0NNS3sa1sz1zVZnp7gaRio2FujOQM/view?usp=sharing';

async function handleDownload(e, platform) {
  e.preventDefault();
  const link = e.currentTarget || document.getElementById('dl-windows-btn');
  const href = link?.dataset?.downloadUrl || link?.getAttribute('href') || WINDOWS_DOWNLOAD_URL;
  const fallbackHref = link?.dataset?.fallbackUrl || WINDOWS_DOWNLOAD_FALLBACK_URL;
  ...
  const interval = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      setTimeout(() => {
        ...
        window.location.href = href || fallbackHref;
      }, 300);
    }
    bar.style.width = progress + '%';
  }, 120);

  await trackEvent('download', { platform, version, file: 'SyariahAppSetup.exe', url: href });
}
